...
namespace Unity.Mathematics.FixedPoint
{
    public static partial class fpmath
    {
        ...
        { 
            // NOTICE: This function was generated by Chat-GPT 4
            if (x == fp.zero)
            {
                return fp.one;
            }
            
            // If x is negative, calculate Exp(-x) and take the reciprocal
            if (x < fp.zero)
            {
                return fp.one / exp(-x);
            }

            // Limit the number of iterations to prevent overflow
            // This number can be adjusted based on the precision of your fixed-point representation
            int maxIterations = 10; 
            
            fp result = fp.one;
            fp factorial = fp.one; // This will be 'i!' in the series
            fp powerOfX = fp.one; // This will be 'x^i' in the series

            for (int i = 1; i <= maxIterations; i++)
            {
                factorial *= (fp)i; // factorial = i!
                powerOfX *= x; // powerOfX = x^i
                result += powerOfX / factorial; // Add the term to the result
            }
